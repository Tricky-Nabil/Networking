...
public:
    ...
    void Setup(Ptr<Socket> socket , Address addr , uint32_t packetSize , DataRate dataRate , uint32_t simulationtime);
...
MyApp::MyApp():m_socket(0),m_peer(),m_packetSize(0),m_dataRate(0),m_sendEvent(),m_running(false),m_packetsSent(0),m_simulationtime(0){}
...
void MyApp::Setup(Ptr<Socket> socket , Address addr , uint32_t packetSize , DataRate dataRate , uint32_t simulationtime){
    m_socket = socket;
    m_peer = addr;
...
static void Cong_Wndw_Change(Ptr<OutputStreamWrapper> stream , uint32_t oldCwnd , uint32_t newCwnd){
...
int main(int argc , char *argv[]){
    ...
    int cleanupTime = 2;
    int bottleneck_rate = 50;
    int bottleneck_delay = 100;
    int packet_loss_exp = 6;
    int exp = 1;
    double bandwidth_delay_product = (1.0) * (bottleneck_delay * bottleneck_rate * 125 ) / payloadSize ;
    ...
    std::string output_folder = "scratch/ns3_2";
    

    // input from CMD
    CommandLine cmd(__FILE__);
    cmd.AddValue("tcp_part_2","Name of TCP variant 2", tcpVariant2);
    //cmd.AddValue("nLeaf","Number of left and right side leaf nodes", nLeaf);
    cmd.AddValue("bottleneck_rate","Max Packets allowed in the device queue", bottleneck_rate);
    cmd.AddValue("packetloss_exp", "Packet loss rate", packet_loss_exp);
    //cmd.AddValue("output_folder","Folder to store dara", output_folder);
    //cmd.AddValue("exp","1 for bttlnck, 2 for packet loss rate", exp);
    cmd.Parse(argc,argv);

    //std::string fileName = output_folder + "/data_" + std::to_string(exp) + ".txt";

    nFlows = nLeaf;
    double packet_loss_rate = (1.0) * (1.0 / std::pow(10, packet_loss_exp));

    std::string bottleNeckDelay = std::to_string(bottleneck_delay) + "ms";
    std::string bottleNeckDataRate = std::to_string(bottleneck_rate) + "Mbps";
    ...
    PointToPointDumbbellHelper d(nLeaf, p2pLeaf,
                                  nLeaf, p2pLeaf,
                                  bottleneck_link);
    ...
    for(int i = 0 ; i < nFlows ; i++){
        ...
        ns3TcpSocket->TraceConnectWithoutContext("CongestionWindow" , MakeBoundCallback(&Cong_Wndw_Change, stream));
    }
    ...
    CurThroughputArr[0] = CurThroughputArr[0] / ((simulationTimeInSeconds + cleanupTime)*1000);
    CurThroughputArr[1] = CurThroughputArr[1] / ((simulationTimeInSeconds + cleanupTime)*1000);
    ...
    // NS_LOG_UNCOND("#######################################################################\n\n");
    NS_LOG_UNCOND("CurThroughputArr[0] : " << CurThroughputArr[0]);
    NS_LOG_UNCOND("CurThroughputArr[1] : " << CurThroughputArr[1]);

    std::string output_file = "scratch/ns3_2/data.txt";
    std::ofstream out(output_file, std::ios::app);
    out << bottleneck_rate << "\t" << packet_loss_exp << "\t" << CurThroughputArr[0] << "\t" << CurThroughputArr[1] << "\t" << std::endl;
